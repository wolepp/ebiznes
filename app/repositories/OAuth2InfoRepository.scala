...
class OAuth2InfoRepository @Inject() (dbConfigProvider: DatabaseConfigProvider)(
  ...
  override def add(loginInfo: LoginInfo, authInfo: OAuth2Info): Future[OAuth2Info] =
    db.run {
        (oAuth2Info
          .map(c => (c.providerId, c.providerKey, c.accessToken, c.tokenType, c.expiresIn))
          .returning(oAuth2Info.map(_.id))
          .into {
            case ((providerId, providerKey, accessToken, tokenType, expiresIn), id) =>
              OAuth2InfoDto(id, providerId, providerKey, accessToken, tokenType, expiresIn)
